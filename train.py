...
class Train():
    def __init__(self):
        self.nb_epoch = 200
        self.data_base = "MIMIC"     # MIMIC, eICU
        self.model_name = "Resnet"   # CNN, Resnet, MLP
        self.nb_layer = 18
        
        # self.data_base=="mimic"
        self.batch_size = 16
        self.input_dim = 16
        self.path = "mimic_data.npz"
        #
    ...
    def cross_validation(self, x, y, pat_num):
        ...
        for train, test in kFold.split(x, y):
            ...
            early_stop = EarlyStopping(monitor='val_loss', min_delta=0, patience=30, verbose=0, mode='min', baseline=None)
            reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=10, mode='min', min_delta=0.000001)
            checkpoint.append(ModelCheckpoint(filepath='./model_path/model%d.h5' % i, monitor='val_acc', save_best_only=True,
                                save_weights_only=False, mode='max', period=1))
            m = set_model(self.data_base, self.model_name, pat_num, self.nb_layer, 16)
            model = m.run()
